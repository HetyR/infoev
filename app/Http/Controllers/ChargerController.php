...
class ChargerController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($searchHistory) {
            ...
            $shouldUpdate = $lastUpdated->diffInHours(now()) > 24; // Ubah ke 24 jam untuk pembaruan lebih sering
            ...
            foreach ($nearbyResults as $place) {
                ...
                if ($placeId) {
                    ...
                    $filteredPlace = [
                        ...
                        'vicinity' => $place['vicinity'] ?? 'Alamat tidak tersedia',
                        'rating' => $place['rating'] ?? null,
                        'business_status' => $place['business_status'] ?? null,
                        'geometry' => [
                            'location' => [
                                'lat' => $place['geometry']['location']['lat'] ?? -7.830843,
                                'lng' => $place['geometry']['location']['lng'] ?? 112.0098111,
                            ]
                        ]
                    ];
