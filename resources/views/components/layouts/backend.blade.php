        ...
        <nav class="bg-white border-b border-gray-200 shadow-sm" x-data="{ open: false, profileOpen: false }">
                ...
                <div class="flex justify-between h-16 items-center">
                    ...
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('backend.dashboard') }}"
                            class="text-xl font-bold text-indigo-600 hover:text-indigo-800 transition">
                            Admin Panel
                        </a>
                        ...
                        @if (Route::is($menu['route'])) text-indigo-600 after:absolute after:left-0 after:bottom-0 after:w-full after:h-0.5 after:bg-indigo-500
                        @else
                            text-gray-600 hover:text-indigo-500 hover:after:absolute hover:after:left-0 hover:after:bottom-0 hover:after:w-full hover:after:h-0.5 hover:bg-indigo-300 @endif">
                                    {{ $menu['name'] }}
                                </a>
                            @endforeach
                        </div>
            ...
            <div x-show="open" x-transition class="md:hidden bg-white px-4 pt-2 pb-4 space-y-1">
               ...
               @else text-gray-600 hover:text-indigo-500 @endif">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="block w-full text-left text-sm text-gray-600 hover:text-red-600 py-2">
                        Logout
                    </button>
                </form>
            </div>
