<x-layouts.backend title="Edit Affiliate">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('backend.affiliate.update', ['affiliate' => $affiliate->id]) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="marketplace" id="marketplace" required>
                                ...
                                @foreach ($marketplaces as $marketplace)
                                    <option value="{{ $marketplace->id }}" @if ($marketplace->id == $affiliate->marketplace->id) selected @endif>
                                        {{ $marketplace->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="desc" name="desc" value="{{ $affiliate->desc }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="price" class="form-label">Price</label>
                            <input type="text" class="form-control" id="price" name="price" value="{{ $affiliate->price }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="link" class="form-label">Link</label>
                            <input type="url" class="form-control" id="link" name="link" value="{{ $affiliate->link }}" required>
                        </div>
