<x-layouts.backend title="Vehicle List">
    <div class="row mb-4">
        <div class="col-md-12 text-end">
            <a href="{{ route('backend.vehicle.create') }}" class="btn btn-outline-success hover-shadow">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    <select id="marketplaceFilter" class="form-select" aria-label="Filter by Marketplace">
                        <option value="" {{ request('marketplace_id') === null ? 'selected' : '' }}>Semua
                            Marketplace</option>
                        <option value="none" {{ request('marketplace_id') === 'none' ? 'selected' : '' }}>Tidak Ada
                            Marketplace</option>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex flex-wrap gap-1 justify-content-center">
                                    ...
                                    <form class="d-inline"
                                        action="{{ route('backend.vehicle.destroy', ['vehicle' => $vehicle->slug]) }}"
                                    ...
                                    <a href="{{ route('vehicle.show', ['vehicle' => $vehicle->slug]) }}"
                                        class="btn btn-sm btn-outline-secondary hover-shadow" target="_blank"
                                        rel="noopener">
                            ...
                            <td>
                                @foreach ($vehicle->affiliate as $affiliate)
                                    <span
                                        class="badge bg-light text-dark">{{ $affiliate->marketplace->name ?? 'N/A' }}</span>
                                @endforeach
                            </td>
                        </tr>
                    @endforeach
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                function updateUrlFilter() {
                    ...
                    typeFilter.value ? url.searchParams.set('type_id', typeFilter.value) : url.searchParams.delete(
                        'type_id');
                    brandFilter.value ? url.searchParams.set('brand_id', brandFilter.value) : url.searchParams.delete(
                        'brand_id');
                    marketplaceFilter.value ? url.searchParams.set('marketplace_id', marketplaceFilter.value) : url
                        .searchParams.delete('marketplace_id');
